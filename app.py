...
def create_radar_chart(scores, labels, title="Cultural Profile"):
    ...
    labels_closed = list(labels) + [list(labels)[0]]
...
def page_test_administration():
    ...
    # Display question
    st.markdown(f"### {current_item['dimension']}")
    ...
    if target_country:
        ...
        for test_name in st.session_state.selected_tests:
            ...
            fig.update_layout(
               ...
               showlegend=True,
                     legend=dict(orientation="h", yanchor="bottom", y=1.05, xanchor="center", x=0.5, font=dict(color='#C9A96E', size=12)),
                plot_bgcolor='#1B2838',
                paper_bgcolor='#1B2838'
            )
